...
//		ptrUsartHandler->ptrUSARTx->BRR = 0x0364;
		...
		case USART_BAUDRATE_230400:
		{
			...
			// valora cargar 4.3124,   4=0x4, 0.3125*16 = 5, 5=0x5
...
static void usart_config_mode(USART_Handler_t *ptrUsartHandler){
	...
	case USART_MODE_TX:
	{
		...
		// Se limpia el registro de recepción RE
		ptrUsartHandler->ptrUSARTx->CR1 &= ~USART_CR1_RE;

		// Se escribe un 1 en el bit 3
	...
	case USART_MODE_RX:
	{
		...
		// Se limpia el bit de transmisión TE del CR1
		ptrUsartHandler->ptrUSARTx->CR1 &= ~USART_CR1_TE;

		// Se escribe un 1 en el bit 2 del CR1
		ptrUsartHandler->ptrUSARTx->CR1 |= USART_CR1_RE;
		break;
	}
	case USART_MODE_RXTX:
	{
		...
		// Se escribe un 1 en el bit 3 del CR1
		ptrUsartHandler->ptrUSARTx->CR1 |= USART_CR1_TE;
		break;
	}
	case USART_MODE_DISABLE:
	{
		...
		// Se escribe un 0 en el bit 2 del CR1
		ptrUsartHandler->ptrUSARTx->CR1 &= ~USART_CR1_RE;

		// Se escribe un 0 en el bit 3 del CR1
		ptrUsartHandler->ptrUSARTx->CR1 &= ~USART_CR1_TE;

		// UE USART ENABLE, SE DESACTIVA
...
void USART1_IRQHandler(void){
	// Evaluamos si la interrupción que se dio es por RX
   	if (USART2->SR & USART_SR_RXNE){
   		auxRxData_Usart1 = (uint8_t) USART2->DR;
   		usart1_RxCallback();
   	}
}
