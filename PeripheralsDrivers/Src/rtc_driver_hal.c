...
void date_and_time_selection(RTC_Handler_t *pRTC_handler);
void RTC_enable(void);
void LSE_clock_enable(void);
void power_interface_enable(void);

/* configuración del RTC*/
void RTC_config(RTC_Handler_t *pRTC_handler){

	/* activa el módulo en el RCC del APB1*/
	power_interface_enable();

	/*activa la saeñal de 32kHz*/
	LSE_clock_enable();

	/* Habilita la modificacion el RTC poninedo la clave en el registro wpr */
	RTC_enable();

    // sin registro sombra--> si se coloca BYPSHAD  = 1, se tiene lectura directa del
	// registro del RTC
	RTC->CR |= RTC_CR_BYPSHAD;

	/* utiliza los valores iniciales de la fecha y hora y las carga en el TR y DR*/
	date_and_time_selection(pRTC_handler);
...
void power_interface_enable(void){
	...
	RCC->APB1ENR |= RCC_APB1ENR_PWREN; // Habilitar módulo PWR (power interface clock enable)
